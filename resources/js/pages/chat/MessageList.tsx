...
const MessageList: React.FC<MessageListProps> = ({ messages, ai_model }) => {
  ...
  if (messages.length === 0) {
    return (
      <motion.div
        key="empty-state"
        // Используем variants для красивого поочередного появления
        initial="initial"
        animate="animate"
  ...
  return (
    <div className="space-y-4">
      ...
      <motion.div
          key={message.isUser ? `user-msg-${index}` : message.id}
      ...
      >
          <div className={`relative group max-w-[92%] sm:max-w-[85%] md:max-w-[80%] ${message.isUser ? 'ml-4' : 'mr-4'}`}>
              <div className={`
                  ...
                  /* УБРАЛИ transition-all, оставили только цвета */
                  transition-colors duration-200 
                  ${message.isUser
                    ? 'bg-linear-to-br from-blue-500 to-blue-600 text-white rounded-tr-none border-b border-blue-700'
